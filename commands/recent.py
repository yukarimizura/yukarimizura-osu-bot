...
class RecentCommands(commands.Cog):
    ...
    async def recent(self, ctx: commands.Context, *, username: str = None):
        ...
        if scores is None:
            await ctx.send("Something went wrong while getting recent scores.")
            return

        if not scores:
            await ctx.send(f"**{user['username']}** has no recent plays.")
            return
        ...
        score_value = score.get("total_score", score.get("score", 0))
        ...
        grade_display = GRADE_DISPLAY.get(rank, f"🎯 {rank}")
        ...
        actual_pp_text = f"◆ **{actual_pp:.2f}pp**" if actual_pp is not None else "◆ **PP unavailable**"
        ...
        count_300 = get_hit_stat(statistics, "great", "count_300")
        count_100 = get_hit_stat(statistics, "ok", "count_100")
        count_50 = get_hit_stat(statistics, "meh", "count_50")
        count_miss = get_hit_stat(statistics, "miss", "count_miss")

        # Map completion progress calculation on fail
        completion = None
        ...
        if rank == "F" and total_objects > 0:
            if passed_objects is not None:
                completion = (passed_objects / total_objects) * 100
            else:
                completion = ((count_300 + count_100 + count_50 + count_miss) / total_objects) * 100

        embed = discord.Embed(
            description=(
                f"**[{title}]({beatmap_url})**\n"
                f"↳ **[{difficulty_name}]({beatmap_url}?diff={beatmap['id']})** · `{star_rating:.2f}★`\n\n"
        ...
        embed.add_field(name="Performance", value=actual_pp_text, inline=True)

        combo_value = f"**{combo}x / {max_combo}x** · `{mods}`"
        if completion is not None:
            combo_value += f"\n**{completion:.2f}%** Progress"

        embed.add_field(name="Combo & Mods", value=combo_value, inline=True)
        embed.add_field(
            name="Hit",
            value=(
                f"`{count_300:,}`**x 300** · "
        ...
        if fc_pp is not None and fc_accuracy is not None:
            embed.add_field(
                name="If FC",
                value=f"◆ **{fc_pp:.2f}pp** @ **{fc_accuracy:.2f}%**",
                inline=False
            )

        embed.set_author(name=f"Recent {mode_name} · {user['username']}", icon_url=user["avatar_url"])
...
async def setup(bot):
    await bot.add_cog(RecentCommands(bot))